   ...
   <xsl:function name="f:tokeniseBlocks">
    ...
    <xsl:function name="f:tokeniseBlocks">
        ...
        <xsl:variable name="newType" as="xs:string?" 
            select="if (exists($block/@type)) then $block/@type else $type"/>
            ...
            <xsl:choose>
                <xsl:when test="exists($start) and exists($length)">
                    <xsl:element name="{$name}">
                        <xsl:attribute name="pos" select="$start"/>
                        <xsl:if test="exists($block/@type)">
                            <xsl:attribute name="type" select="$block/@type"/>
                        </xsl:if>
                        <xsl:value-of select="substring($doc-text, $start, $length)"/>
                    </xsl:element>
                </xsl:when>
            </xsl:choose>
            <xsl:if test="$name = ('x-tag-end', 'xml-tag')">
                <xsl:variable name="newStart" select="$start + $length"/>
                <x-content start="{$newStart}" type="{$type}">
                    <xsl:if test="$name eq 'xml-tag'">
                        <xsl:attribute name="type" select="$newType"/>
                    </xsl:if>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    ...
                    <xsl:when test="$xmlStarts">
                        ...
                        <xsl:sequence select="f:createXmlBlocks($chars, $index + 1, (), $index, $xLevel, $fnLevel, ($final-result, $end))"/>                       
                    </xsl:when>
                    <xsl:otherwise>
