...
<!--        <xsl:variable name="doc-text" as="xs:string" select="unparsed-text('file:///c:/test/XQueryML30.xq')"/>-->
   ...
   <xsl:function name="f:tokeniseBlocks">
    ...
    <xsl:function name="f:tokeniseBlocks">
        ...
        <xsl:variable name="current-is-preceding-literal" as="xs:boolean"
                      select="if ($name eq 'comment') then
                                $is-preceding-literal
                              else ($name eq 'literal')"/>
        <xsl:variable name="xq-token" as="element()*"
            select="f:handleXquery(substring($doc-text, $followingStart, $nextStart - $followingStart), $current-is-preceding-literal)"/>
        
       <!-- todo: must check if whitespace found -->
        <xsl:variable name="new-is-preceding-literal" as="xs:boolean"
            select="$current-is-preceding-literal and empty($xq-token)"/>
        
        <xsl:variable name="result1" as="element()*">
            <xsl:if test="$index eq 1 and not($start = 1)">
                 <xquery pos="1">
                    <xsl:sequence select="f:handleXquery(substring($doc-text, 1, $start - 1), false())"/>
                </xquery>               
            </xsl:if>
            <xsl:choose>
                    ...
                    <xsl:choose>
                        <xsl:when test="$name eq 'start-xquery'">
                            <xquery pos="{$shiftStart}">
                                <xsl:sequence select="f:handleXquery($text, false())"/>
                            </xquery>
                        </xsl:when>
                ...
                <xsl:when test="$name = ('x-tag-end', 'xml-tag')">
                    <xsl:if test="$followingStart ne $nextStart">
                         <span pos="{$followingStart}" 
                            ...
                            <xsl:value-of select="substring($doc-text, $followingStart, $nextStart - $followingStart)"/>
                        </span>                   
                    </xsl:if>
                </xsl:when>
            </xsl:choose>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:sequence select="f:tokeniseBlocks($doc-text, $blocks, $index + 1, $newType, $final-result, $new-is-preceding-literal)"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    ...
    <xsl:function name="f:handleXquery" as="element()*">
        <xsl:param name="text"/>
        <xsl:param name="is-preceding-literal" as="xs:boolean"/>
        <handled follows-literal="{if ($is-preceding-literal) then 'yes' else 'no'}">
            <xsl:sequence select="$text"/>
        </handled>
    </xsl:function>
