       ...
       <!-- todo: must check if whitespace found -->
        ...
        <xsl:variable name="result1" as="element()*">
            ...
            <xsl:if test="$followingStart ne $nextStart">
                <!-- kilroy -->
                <xsl:element name="{concat('following-',$name)}">
                     <xsl:attribute name="current-closed" select="string($current-is-preceding-closed)"/>
                     <xsl:attribute name="new-closed" select="string($new-is-preceding-closed)"/>
                </xsl:element>
                <xsl:choose>
...
<!--       <xsl:variable name="test-string" select="codepoints-to-string(subsequence($chars, $index + 1, 10))"/>-->
        ...
        <xsl:choose>
            ...
            <xsl:when test="($level eq 0 and $char eq $cTagEnd) or ($level eq 1 and $other-tag-end)">
                <xsl:variable name="end" as="element()">
                 <start-xquery pos="{$index + ($limit + 1)}" level="top"/>                     
                </xsl:variable>  
                <xsl:sequence select="f:createXqBlocks($chars, false(), $index + 1, (), 0, 0, $xq-fnlevel, 0, $nowAwaiting, ($final-result, $end))"/>                 
            </xsl:when>
