...
<!--        <xsl:variable name="doc-text" as="xs:string" select="unparsed-text('file:///c:/test/XQueryML30.xq')"/>-->
   ...
   <xsl:function name="f:tokeniseBlocks">
    ...
    <xsl:function name="f:tokeniseBlocks">
        ...
        <xsl:variable name="text" select="if (empty($start) or empty($length)) then '' 
            else substring($doc-text, $shiftStart, $length - $offset)"/>
        <xsl:variable name="following-string" select="if (empty($followingStart) or empty($nextStart)) then '' 
       ...
       <!-- todo: must check if whitespace found -->
        ...
        <xsl:variable name="result1" as="element()*">
            <xsl:if test="$index eq 1 and not($start = 1)">
                 <!--<xquery pos="1"/>-->
                 <xsl:sequence select="xq:createXqTokens(substring($doc-text, 1, $start - 1), false())"/>            
            </xsl:if>
            <xsl:choose>
                <xsl:when test="exists($start) and exists($length)">
                    ...
                    <xsl:choose>
                        <xsl:when test="$name eq 'start-xquery'">
                            <xsl:if test="$shiftStart ne $nextStart">
                            ...
                            <xsl:choose>
                                <xsl:when test="$name = 'literal'">
                                    ...
                                    <span class="op"><xsl:value-of select="$quote"/></span>
                                     <span class="{$name}" pos="{$shiftStart}" xq="yes">
    ...
    <xsl:variable name="class-out" as="xs:string*"
     ...
     <xsl:variable name="name-in" as="xs:string*"
        select="('nested-query','unnested-xquery', 'xml-open-tag','xml-name-end', 'x-equals',
            'xml-att-quote', 'xml-literal-start', 'xml-literal-end', 'resume-x-literal','xml-close-tag')"/>
    ...
    <!--            <xsl:otherwise>
        ...
        <xsl:variable name="isEmbeddedXQuery" as="xs:boolean"
            select="$char eq $cFnStart and $n1Char ne $cFnStart and $awaiting = ($cApos, $cQuote, $cTagStart)"/>
        
        <xsl:variable name="isUnEmbeddedXQuery" as="xs:boolean"
            select="$char eq $cFnEnd and $n1Char ne $cFnEnd and $awaiting = ($cApos, $cQuote, $cTagStart)"/>
...
<!--       <xsl:variable name="test-string" select="codepoints-to-string(subsequence($chars, $index + 1, 10))"/>-->
        ...
        <xsl:variable name="result2" as="element()*">
            <xsl:choose>
                ...
                <xsl:when test="$isOpenTag">
                    <xml-open-tag pos="{if ($char eq $cTagStart) then $index else $index - 1}"  xxq-fnlevel="{string($xq-fnlevel)}"/>                    
                </xsl:when>
                <xsl:when test="$isEmptyTag">
                    <x-tag-end pos="{$index}" length="2" level="{$level}" xq-fnlevel="{$xq-fnlevel}"/>
                </xsl:when>
                <xsl:when test="$isCloseTag">
        ...
        <xsl:choose>
            ...
            <!--<xsl:when test="($level eq $xml-stack-level and $char eq $cTagEnd) or ($level eq $xml-stack-level + 1 and $other-tag-end)">-->
                ...
                <xsl:variable name="end" as="element()">
                 <start-xquery pos="{$index + $offset + 1}" xml-exit-level="{$newLevel}" offset="{$offset}"/>                     
                </xsl:variable>
                <xsl:variable name="popped-stack" as="xs:integer*" 
                              select="subsequence($xml-stack, 1, count($xml-stack) - 1)"/>
                <xsl:sequence select="f:createXqBlocks($chars, false(), $index + 1, (), 0, 0, $xq-fnlevel, $newLevel, $nowAwaiting, $popped-stack, ($final-result, $end))"/>                 
            </xsl:when>
            <xsl:when test="$isEmbeddedXQuery or $isUnEmbeddedXQuery">
                <xsl:variable name="end" as="element()+">
                <nested-query pos="{$index}" down="{string($isEmbeddedXQuery)}"/>
                <start-xquery pos="{$index + 1}"/>
                </xsl:variable>  
                <xsl:sequence select="f:createXqBlocks($chars, false(), $index + 1, (), 0, 0, $xq-fnlevel, $newLevel, $nowAwaiting, $xml-stack, ($final-result, $end))"/>               
            </xsl:when>
            <xsl:otherwise>
                <xsl:sequence select="f:createXmlBlocks($chars, $index + 1, $nowAwaiting, $start, $newLevel, $xq-fnlevel, $xml-stack, $final-result)"/>                
            </xsl:otherwise>
        </xsl:choose>
                ...
                <xsl:choose>
                    <xsl:when test="$xqueryEnds">
                        ...
                        <xsl:sequence select="f:createXmlBlocks($chars, $index + 1, $xAwaiting, $index, $xLevel,
                                                                0, $xml-stack, ($final-result, $end))"/>                       
                    </xsl:when>
                    <xsl:when test="$xmlStarts">
                        <xsl:variable name="end" as="element()">
                            <xml-constructor pos="{$index}" stack="{concat(string-join((for $x in $xml-stack return string($x)), ':'), '+', $xLevel)}"/>
                        </xsl:variable>
                        <xsl:sequence select="f:createXmlBlocks($chars, $index + 1, (), $index, $xLevel,
                                                                $fnLevel, ($xml-stack, $xLevel), ($final-result, ()))"/>                       
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:sequence select="f:createXqBlocks($chars, $nowSkip, $index + 1, $nowAwaiting, $newStart, 
                                                               $newLevel, $newFnLevel, $xLevel, $xAwaiting, $xml-stack, $final-result)"/>                           
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
        </xsl:choose>
