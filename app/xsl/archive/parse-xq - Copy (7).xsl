...
<!--        <xsl:variable name="doc-text" as="xs:string" select="unparsed-text('file:///c:/test/XQueryML30.xq')"/>-->
   ...
   <xsl:function name="f:tokeniseBlocks">
        ...
        <xsl:sequence select="f:tokeniseBlocks($doc-text, $blocks, 1, 1, '', (), false())"/>
   </xsl:function>
    ...
    <xsl:function name="f:tokeniseBlocks">
        ...
        <xsl:variable name="result1" as="element()*">
            ...
            <xsl:variable name="xq-token" as="element()*"
                select="f:handleXquery(substring($doc-text, $newStart, $nextStart - $newStart), $current-is-preceding-literal)"/>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:sequence select="f:tokeniseBlocks($doc-text, $blocks, $index + 1, $newStart, $newType, $result1, $new-is-preceding-literal)"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
